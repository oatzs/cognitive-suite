<script lang="ts">
  ...
  const engine = createEngine(undefined, (session) => onSessionComplete(session));
  let gameState = $state<GameState>(engine.getState());
  ...
  const isActive = $derived(gameState.phase === 'active' || gameState.phase === 'paused' || gameState.phase === 'ending');
  ...
  onMount(() => {
    for (const session of engine.loadHistory()) onSessionComplete(session);
  });
...
<div data-app-shell class="docct-app relative flex w-full h-full z-2 overflow-auto bg-[#090a0d]">
  ...
  <main aria-hidden={gameState.phase === 'onboarding' || historyOpen} inert={gameState.phase === 'onboarding' || historyOpen ? true : undefined} class="flex flex-col grow md:px-6 md:items-center {isActive ? 'select-none' : ''}">
    ...
    <div class="md:flex md:py-6 justify-between gap-2 w-full max-w-7xl">
      ...
      <div class="{isActive ? 'flex px-4 pt-4' : 'hidden'} gap-6 items-center md:flex md:px-0 md:pt-0">
        {#if isActive}
          <button class="cursor-pointer flex gap-2 items-center bg-[#a9b4cc] hover:bg-[#ffffff] p-1 px-4 rounded-md" onclick={() => engine.quit()} disabled={gameState.phase === 'ending'}>
            <span class="text-[#090a0d] text-xs font-semibold">QUIT SESSION</span>
          </button>
        {/if}
        <!-- Timer stays visible during setup; Focus hides it only while training. -->
        {#if !isActive || gameState.settings.displayMode === 'standard'}
          <div data-session-timer class="flex items-center gap-3">
            ...
            <div class="flex h-[30px] items-center rounded-md border border-[#7e889c] bg-[#0f121a] px-4">
              <span class="text-xs font-extrabold text-[#ffffff]">{formatTime(gameState.timeLeft)}</span>
            </div>
          </div>
        {/if}
      </div>
      ...
      <div class="{isActive ? 'hidden md:flex' : 'flex'} pt-6 pb-4 md:pt-0 md:pb-0 flex-col md:flex-row gap-2 items-center">
          <!-- DIGIT dropdown -->
          <div class="relative">
            <button aria-expanded={digitDropdownOpen} aria-controls="digit-options" aria-haspopup="true" class="cursor-pointer flex items-center p-1 px-4 border border-[#a9b4cc] gap-4 rounded-md" disabled={isActive} onclick={() => { digitDropdownOpen = !digitDropdownOpen; answerDropdownOpen = false; }}>
              <span class="text-[#7e889c]">DIGIT</span>
              <div class="flex items-center gap-1">
                <span class="text-[#a9b4cc] font-medium">{gameState.settings.useVoice ? 'Voice' : 'Text'}</span>
            ...
            {#if digitDropdownOpen}
              <div id="digit-options" role="group" aria-label="Digit presentation" class="absolute flex top-[42px] left-0 w-full pt-3 z-2">
                <div class="flex grow flex-col bg-[#a9b4cc] p-1 rounded-md">
                  <button aria-pressed={gameState.settings.useVoice} class="group min-h-11 hover:bg-[#000000] rounded-md cursor-pointer flex p-2 justify-end" onclick={() => { engine.updateSettings({ useVoice: true }); digitDropdownOpen = false; }}>
                    <span class="text-[#090a0d] group-hover:text-[#a9b4cc] font-medium">Voice</span>
                  </button>
                  <button aria-pressed={!gameState.settings.useVoice} class="group min-h-11 hover:bg-[#000000] rounded-md cursor-pointer flex p-2 justify-end" onclick={() => { engine.updateSettings({ useVoice: false }); digitDropdownOpen = false; }}>
          ...
          <!-- ANSWER dropdown -->
          <div class="relative">
            <button aria-expanded={answerDropdownOpen} aria-controls="answer-options" aria-haspopup="true" class="cursor-pointer flex items-center p-1 px-4 border border-[#a9b4cc] gap-4 rounded-md" disabled={isActive} onclick={() => { answerDropdownOpen = !answerDropdownOpen; digitDropdownOpen = false; }}>
              <span class="text-[#7e889c]">ANSWER</span>
              <div class="flex items-center gap-1">
                <span class="text-[#a9b4cc] font-medium">{gameState.settings.useKeypad ? 'On-screen keypad' : 'Keyboard'}</span>
            ...
            {#if answerDropdownOpen}
              <div id="answer-options" role="group" aria-label="Answer method" class="absolute flex top-[42px] left-0 w-full pt-3 z-2">
                <div class="flex grow flex-col bg-[#a9b4cc] p-1 rounded-md">
                  <button aria-pressed={gameState.settings.useKeypad} class="group min-h-11 hover:bg-[#000000] rounded-md cursor-pointer flex p-2 justify-end" onclick={() => { engine.updateSettings({ useKeypad: true }); answerDropdownOpen = false; }}>
                    <span class="text-[#090a0d] group-hover:text-[#a9b4cc] font-medium">On-screen keypad</span>
                  </button>
                  <button aria-pressed={!gameState.settings.useKeypad} class="group min-h-11 hover:bg-[#000000] rounded-md cursor-pointer flex p-2 justify-end" onclick={() => { engine.updateSettings({ useKeypad: false }); answerDropdownOpen = false; }}>
        ...
        <div class="relative flex">
          <button aria-expanded={settingsOpen} aria-controls="settings-panel" aria-haspopup="true" class="cursor-pointer flex items-center p-1 px-4 border border-[#a9b4cc] gap-4 rounded-md" disabled={isActive} onclick={() => settingsOpen = !settingsOpen}>
    ...
    <div class="flex grow">
      <div class="flex grow md:pb-[122px] md:justify-center">
        ...
        {:else if gameState.phase === 'setup'}
          <Setup {engine} />
        {:else if gameState.phase === 'active' || gameState.phase === 'paused'}
