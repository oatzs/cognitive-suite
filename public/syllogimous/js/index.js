...
function playSound(property) {
    if (appState.sfx === 'none') return;
    const AudioContext = window.AudioContext || window.webkitAudioContext;
    if (!AudioContext) return;
    const frequencies = appState.sfx === 'sfx2'
        ? { success: 523, failure: 220, missed: 165 }
        : { success: 660, failure: 196, missed: 147 };
    const context = new AudioContext();
    const oscillator = context.createOscillator();
    const gain = context.createGain();
    oscillator.frequency.value = frequencies[property];
    oscillator.type = appState.sfx === 'sfx2' ? 'sine' : 'triangle';
...
function resetApp() {
    ...
    if (confirmed) {
        window.dispatchEvent(new Event('syllogimous-reset'));
        if (window.parent !== window) {
            window.parent.postMessage({
                source: 'cognitive-suite:syllogimous',
                type: 'data-reset',
            }, '*');
        }
